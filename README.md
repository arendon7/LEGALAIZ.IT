...
> M33.1 conserva íntegramente M33.0 y M32.9 y endurece el despliegue de la producción demostrativa pública: elimina credenciales fijas de la configuración activa, activa control de origen, mantiene cookies seguras bajo HTTPS, deriva la llave de cifrado desde un secreto administrado por el proveedor y condiciona el autodespliegue a CI satisfactoria. No acredita aprobación profesional de asuntos reales, representación judicial, radicación, firma, notificación externa ni entrega efectiva.
...
- Blueprint Render con plan explícito, despliegue tras checks verdes y secretos fuera de la configuración versionada.
...
4. Ejecuta `01_INICIAR_DEMO_PUBLICA_LINUX.sh`.
5. Usa la contraseña de sesión mostrada en la terminal y abre el puerto publicado por el entorno.
...
Si `LEGAL_DEMO_PASSWORD` no está definida, los iniciadores locales generan una contraseña aleatoria para esa sesión y la muestran únicamente en la terminal local. Los iniciadores y configuraciones activas no contienen una contraseña fija; cualquier referencia heredada de compatibilidad se neutraliza antes de las respuestas M33.

### Docker / Render

- `deploy/docker-compose.public-demo.yml` levanta la demo con Docker Compose y exige `LEGAL_DEMO_PASSWORD` desde el entorno.
...
- Render solicita `LEGAL_DEMO_PASSWORD` al crear el Blueprint; el valor no se almacena en la configuración versionada.
...
**Contraseña:** valor configurado en `LEGAL_DEMO_PASSWORD`. La API M33 no devuelve ese valor.
...
- `ci.yml`: sintaxis, suite completa, 11 productos, cobertura de formularios, arranque HTTP, smoke M33 con rechazo de origen no autorizado, no exposición de contraseña y renderizado DOCX;
