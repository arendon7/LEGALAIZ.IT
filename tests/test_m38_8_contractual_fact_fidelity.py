...
class M388ContractualFactFidelityTests(unittest.TestCase):
    def test_rich_services_answers_reach_material_contract_clauses(self):
        ...
        self.assertTrue(composition["maturity_answers"]["m38_8_contractual_fact_fidelity"])
    ...
    def test_optional_empty_or_sentinel_facts_do_not_leak_into_contract(self):
        ...
        answers["execution"]["team"] = ""
        text = _visible_text(compose_services_m33_release(answers))

        self.assertNotIn("medidas de seguridad específicamente confirmadas para la ejecución se aplicarán: undefined", text)
        self.assertNotIn("aseguramiento se registró como condición de gestión: N/A", text)
        self.assertNotIn("condición confirmada: NULL", text)
        self.assertNotIn("estructura de equipo confirmada es:", text)
    ...
    def test_fact_fidelity_is_additive_and_keeps_existing_controls(self):
        text = _visible_text(compose_services_m33_release(_services_answers()))
