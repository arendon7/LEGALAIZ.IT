...
class CoEm003GovernanceV243:
    VERSION = "2.43"
    """Immutable revisions, comparison, dual approval and controlled release."""
    ...
    def register_generation(self, result, answers, actor):
        ...
        if not self._revision_path(generation_id, 1).exists():
            ...
            snapshot = self._snapshot(
                generation_id, 1, answers, actor, None, "Generación documental inicial.",
                evaluation, manifest.get("documents", []), manifest.get("hashes", {}),
            )
    ...
    def _flatten(value, prefix=""):
        ...
        if isinstance(value, dict):
            for key in sorted(value):
                out.update(CoEm003GovernanceV243._flatten(value[key], f"{prefix}.{key}" if prefix else key))
    ...
    def compare(self, generation_id, from_revision, to_revision):
        ...
        return {
            ...
            "changes": changes,
            "document_changes": {
                "before": left.get("documents", []),
                "after": right.get("documents", []),
            },
        }
