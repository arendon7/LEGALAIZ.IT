...
class CoLa001GovernanceV252:
    ...
    def _snapshot(self, generation_id, revision_number, answers, actor, base_revision, change_note, evaluation, documents, hashes, calculation_summary=None):
        snapshot = {
            "generation_id": generation_id,
            ...
            "review_requirements": evaluation.get("professional_reviews", evaluation.get("review_requirements", [])),
    ...
    def register_generation(self, result, answers, actor):
        ...
        if not self._revision_path(generation_id, 1).exists():
            ...
            snapshot = self._snapshot(generation_id, 1, answers, actor, None, "Generación documental inicial.", evaluation, manifest.get("documents", []), manifest.get("hashes", {}), manifest.get("calculation_summary", {}))
    ...
    def create_revision(self, generation_id, answers, actor, base_revision, change_note):
        ...
        rendered = self.factory.render_documents(answers, target)
        evaluation, documents, hashes, calculation = rendered
        calculation_summary = {
            "gross_total": calculation.get("gross_total"),
            "gross_total_formatted": self.factory._money(calculation.get("gross_total")),
            "prior_payments_total": calculation.get("prior_payments_total"),
            "prior_payments_formatted": self.factory._money(calculation.get("prior_payments_total")),
            "net_total": calculation.get("net_total"),
            "net_total_formatted": self.factory._money(calculation.get("net_total")),
            "moratory_reference_not_added": calculation.get("moratory_reference_not_added"),
            "moratory_reference_formatted": self.factory._money(calculation.get("moratory_reference_not_added")),
        }
        snapshot = self._snapshot(generation_id, new_number, answers, actor, current, str(change_note).strip(), evaluation, documents, hashes, calculation_summary)
        self._write_snapshot(generation_id, snapshot)
        manifest.update({
    ...
    def _flatten(value, prefix=""):
        ...
        if isinstance(value, dict):
            for key in sorted(value):
                out.update(CoLa001GovernanceV252._flatten(value[key], f"{prefix}.{key}" if prefix else key))
