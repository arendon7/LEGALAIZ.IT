...
function publicHeader(active='') {
  const links = [
    ['solutions','Soluciones','/soluciones'], ['finder','Orientador','/orientador'],
...
function recommendedCodes() {
  ...
  if (finderState.goal === 'crear') codes = [...codes].sort((a,b)=>Number(!['CO-LA-002','CO-EM-003','CO-EM-004','CO-AR-001','CO-CD-004'].includes(a))-Number(!['CO-LA-002','CO-EM-003','CO-EM-004','CO-AR-001','CO-CD-004'].includes(b)));
...
function finderContent() {
  ...
  return `<section class="m295-page-hero"><div><span class="public-kicker dark">Orientador de soluciones</span><h1>Empieza por tu situación, no por el nombre del documento.</h1><p>Responde tres decisiones generales. No pediremos nombres, documentos, hechos sensibles ni información del caso.</p></div><img src="/assets/brand-visuals/conversion-compass.svg" alt="Brújula que conecta una necesidad con una solución jurídica"></section>
  <section class="public-section m295-finder" aria-label="Orientador de soluciones">
...
export function createConversionExperience({ app, esc, api, go, toast, shell, pageHeader, state, getPath }) {
  function finderPage() {
    app.innerHTML = publicShell({ active:'finder', title:'Orientador de soluciones', content:finderContent() });
  }
  ...
  async function adminExperiencePage() {
    ...
    const eventLabels = { route_view:'Vistas', cta_click:'Clics en CTA', solution_finder_started:'Orientadores iniciados', solution_finder_completed:'Orientadores completados', lead_form_started:'Formularios iniciados', lead_submitted:'Solicitudes recibidas', performance_sample:'Muestras de rendimiento' };
