...
"""M38.8 · Fidelidad factual del instrumento visible CO-EM-003.

Capa posterior a la composición jurídica M33. No crea reglas jurídicas nuevas ni
sustituye la biblioteca contractual: incorpora en cláusulas existentes hechos materiales
confirmados por el usuario que de otro modo quedarían expresados de forma genérica.
...
def apply_services_fact_fidelity(composition: dict, answers: dict) -> dict:
    ...
    for section in result.get("sections") or []:
        ...
        if _has(section, "ENTREGABLES Y TRAZABILIDAD"):
            _append(section, f"PARÁGRAFO. Como criterio general de aceptación confirmado para este encargo se aplicará {acceptance}." if acceptance else "")
            ...
            _append(section, f"PARÁGRAFO. Para este contrato, la estructura de equipo confirmada es: {execution_team}." if execution_team else "")

        elif _has(section, "SUBCONTRATACIÓN"):
            _append(section, f"PARÁGRAFO. La condición particular informada para subcontratación es: {subcontracting}. Esta precisión se interpreta conjuntamente con las autorizaciones y responsabilidades previstas en la presente cláusula." if subcontracting else "")
            ...
            _append(section, f"PARÁGRAFO. El soporte de cobro pactado para esta operación es: {invoice}." if invoice else "")

        elif _has(section, "GASTOS Y REEMBOLSOS"):
            _append(section, f"PARÁGRAFO. En este contrato LAS PARTES han definido respecto de gastos que {expenses}." if expenses else "")

        elif _has(section, "TRIBUTOS, RETENCIONES"):
            _append(section, f"PARÁGRAFO. La condición particular registrada sobre retenciones y descuentos es: {retentions}." if retentions else "")
            ...
            _append(section, f"PARÁGRAFO. Para las actividades de este contrato LAS PARTES han descrito los roles de tratamiento así: {data_roles}. La calificación jurídica definitiva de cada rol dependerá de la actividad efectivamente realizada y de quién determine sus finalidades y medios esenciales." if data_roles else "")

        elif _has(section, "SEGURIDAD DE LA INFORMACIÓN"):
            _append(section, f"PARÁGRAFO. Como medidas de seguridad específicamente confirmadas para la ejecución se aplicarán: {data_security}." if data_security else "")

        elif _has(section, "PROPIEDAD INTELECTUAL PREEXISTENTE"):
            _append(section, f"PARÁGRAFO. LAS PARTES han identificado la siguiente regla particular sobre activos preexistentes: {ip_preexisting}." if ip_preexisting else "")

        elif _has(section, "RESULTADOS Y DERECHOS PATRIMONIALES"):
            _append(section, f"PARÁGRAFO. Respecto de los resultados específicamente producidos en ejecución del contrato se registró la siguiente asignación: {ip_results}. Esta descripción deberá interpretarse con los requisitos legales de determinación, alcance y forma previstos en esta cláusula." if ip_results else "")

        elif _has(section, "SOFTWARE, COMPONENTES DE TERCEROS"):
            _append(section, f"PARÁGRAFO. Los componentes o materiales de terceros conservarán el siguiente tratamiento confirmado: {ip_third_party}." if ip_third_party else "")

        elif _has(section, "INTELIGENCIA ARTIFICIAL"):
            _append(section, f"PARÁGRAFO. Para este encargo LAS PARTES han establecido adicionalmente la siguiente regla operativa sobre inteligencia artificial: {ai_rules}." if ai_rules else "")
            ...
            _append(section, f"PARÁGRAFO. Como contenido particular de la transición se ha confirmado: {closure_transition}." if closure_transition else "")

        elif _has(section, "LIQUIDACIÓN Y ACTA DE CIERRE"):
            _append(section, f"PARÁGRAFO. Respecto de información, soportes y credenciales al cierre se aplicará la siguiente condición confirmada: {closure_return_destroy}." if closure_return_destroy else "")

        elif _has(section, "SOLUCIÓN ESCALONADA DE CONTROVERSIAS") and dispute:
            visible = _dispute_rule(dispute)
            _append(section, f"PARÁGRAFO. El mecanismo particular seleccionado por LAS PARTES corresponde a {visible}; esta selección no excluye requisitos de procedibilidad ni competencias imperativas que resulten aplicables." if visible else "")

    result.setdefault("maturity_answers", {})["m38_8_contractual_fact_fidelity"] = True
    return result
