...
def _header_footer(doc: Document, footer_text: str) -> None:
    for section in doc.sections:
        ...
        p.add_run("  |  Biblioteca contractual vigente").font.color.rgb = _rgb(MID)
...
def build_professional_docx(path: Path, *, title: str, subtitle: str,
                            ...
                            footer: str = "LegalAIZ.it - Más que respuestas, soluciones.") -> Path:
    path = Path(path)
    ...
    _header_footer(doc, footer)
    props = doc.core_properties
    props.title = title
    props.subject = subtitle
    props.author = "LegalAIZ.it"
    props.last_modified_by = "LegalAIZ.it"
    props.keywords = "LegalAIZ.it, Colombia, contrato, biblioteca contractual versionada"
    props.comments = "Documento generado por la biblioteca contractual versionada de LegalAIZ.it."
...
def build_consolidated_docx(path: Path, *, product_code: str, product_title: str,
                            ...
                            metadata: list[tuple[str, str]], logo_path: Path | None = None) -> Path:
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    doc = Document()
    _configure_styles(doc)
    ...
    _add_cover(
        doc,
        f"{product_code} - PAQUETE CONTRACTUAL PROFUNDO",
        product_title + " · documento principal, anexos y controles integrados",
        metadata,
        logo_path,
    )
    ...
    for pos, (title, subtitle, sections) in enumerate(documents, 1):
        ...
        for sec_index, sec in enumerate(sections):
            ...
            if sec_index == 0 and sec.get("page_break_before"):
                ...
                sec["page_break_before"] = False
            _add_section(doc, sec)
    _header_footer(doc, "LegalAIZ.it - Paquete contractual vigente")
    props = doc.core_properties
    props.title = f"{product_code} - {product_title}"
    props.subject = "Paquete contractual versionado"
    props.author = "LegalAIZ.it"
    props.last_modified_by = "LegalAIZ.it"
    props.created = datetime.now(timezone.utc)
    props.modified = datetime.now(timezone.utc)
